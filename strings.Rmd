...
summary:
digit: `\d`; need backslash to escape
whitespace: `\s`; need backslash to escape
letter: `[abc]`
except letter: `[^abc]`
set boundaries for alteration |
...
```{r}
str_view(words, "^q[a-z]", match = TRUE)
...
```{r}
a <- str_subset(words, "^[aeiou]")
a <- str_subset(a, "[^aeiou]$")
head(a)
```
